document.addEventListener('DOMContentLoaded', () => {
    ...
    const rewardNotice = document.getElementById('reward-notice');
    ...
    const preMatchScenario = [
        ...
        { speaker: '主人公', text: 'はい！行ってきます！' }
    ];

    const events = [
        { id: 1, character: 'のぶ', scenario: [ { speaker: 'のぶ', text: 'おーい！今度のYouTubeの企画で、すごいシュートを決めたいんだ！' }, { speaker: '主人公', text: '面白そうですね！どんなシュートですか？' }, { speaker: 'のぶ', text: 'それは見てのお楽しみだ！手伝ってくれたら、キックのコツを教えてやるよ。' } ], effect: { kick: 15, friendship: 8 } },
        { id: 2, character: 'つばさ', scenario: [ { speaker: 'つばさ', text: '最近、走り込みが足りない気がする…。一緒にトレーニングしないか？' }, { speaker: '主人公', text: 'はい、ぜひ！' }, { speaker: 'つばさ', text: 'よし、じゃあグラウンド10周だ！ついてこいよ！' } ], effect: { speed: 15, friendship: 8 } },
        { id: 3, character: 'ダンガン', scenario: [ { speaker: 'ダンガン', text: 'なあ、俺の必殺ドリブル、見てみたくないか？' }, { speaker: '主人公', text: 'はい！ぜひ見たいです！' }, { speaker: 'ダンガン', text: 'よし、じゃあ特別に見せてやるよ！この動き、目に焼き付けな！' } ], effect: { speed: 10, kick: 5, friendship: 8 } },
        { id: 4, character: 'かにゃ', scenario: [ { speaker: 'かにゃ', text: 'ねえねえ、次の対戦相手の試合ビデオ、一緒に見ない？' }, { speaker: '主人公', text: 'いいですね！分析しましょう。' }, { speaker: 'かにゃ', text: 'このチーム、サイドバックの裏が弱点かも…ふふ、勝つためのヒント、見つけちゃった♪' } ], effect: { iq: 15, friendship: 8 } }
    ];
    ...
    function resetGame() {
        gameState = { playerName: '', week: 1, stats: { kick: 0, speed: 0, iq: 0 }, };
    ...
    function performTraining(trainingType) {
        ...
        let statGains = { kick: 0, speed: 0, iq: 0 }, partner = null, friendshipGain = 0, logMessage = '';
        switch(trainingType) {
        ...
        if (Math.random() < 0.7) {
            ...
            npcChar.src = partner.img; npcChar.style.display = 'block';
            friendshipGain = 5; logMessage += `\n${partner.name}と一緒に練習した！`;
            if (partner.specialty === trainingType) {
                statGains.kick += 8; statGains.speed += 8; statGains.iq += 8;
                friendshipGain += 3; logMessage += `\n得意な練習だったので、さらに効果が上がった！`;
            }
            partner.friendship += friendshipGain;
        }
        gameState.stats.kick += statGains.kick; gameState.stats.speed += statGains.speed; gameState.stats.iq += statGains.iq;
    ...
    function playStory(scenario, npc, onComplete) {
        ...
        function showNextLine() {
            if (currentLine >= scenario.length) { onComplete(); return; }
            const line = scenario[currentLine];
            speakerName.textContent = line.speaker === '主人公' ? gameState.playerName : line.speaker;
            dialogueText.textContent = line.text;
            dialogueBox.className = 'dialogue-box';
            if (line.speaker === '主人公') dialogueBox.classList.add('player-speaking');
            else dialogueBox.classList.add('npc-speaking');
            currentLine++;
        }
        eventNpcChar.src = npc.img;
        switchScreen('event'); showNextLine();
    ...
    function startMatch() {
        ...
        const interval = setInterval(() => {
            if (i < matchSequence.length) {
                const scene = matchSequence[i];
                matchImage.src = scene.image;
                commentaryText.textContent = scene.text;
                i++;
            } else {
        ...
        Object.values(characters).filter(c => c.name !== 'バモンキー').forEach(char => {
            ...
            passButtons.appendChild(button);
        });
    }
    ...
    function showResult(isVictory) {
        ...
        if (isVictory) {
            ...
            let topCharacter = null, maxFriendship = -1;
            Object.values(characters).filter(c => c.name !== 'バモンキー').forEach(char => {
    ...
    function showRewardImage() {
        ...
        rewardTimer = setTimeout(() => {
            playerNameInput.value = ''; init(); switchScreen('start');
        }, 10000);
        ...
        rewardBackgroundImage.onload = () => {
            const containerWidth = screens.reward.clientWidth, containerHeight = screens.reward.clientHeight;
            const imgAspectRatio = rewardBackgroundImage.naturalWidth / rewardBackgroundImage.naturalHeight;
            const containerAspectRatio = containerWidth / containerHeight;
            let canvasWidth, canvasHeight;
            if (imgAspectRatio > containerAspectRatio) {
                canvasWidth = containerWidth; canvasHeight = containerWidth / imgAspectRatio;
            } else {
                canvasHeight = containerHeight; canvasWidth = containerHeight * imgAspectRatio;
            }
            rewardTextCanvas.width = canvasWidth; rewardTextCanvas.height = canvasHeight;
            rewardTextCtx.clearRect(0, 0, rewardTextCanvas.width, rewardTextCanvas.height);
            rewardTextCtx.fillStyle = "white"; rewardTextCtx.strokeStyle = "black"; rewardTextCtx.textAlign = "center";
            
            const nameFontSize = canvasWidth * 0.07;
            rewardTextCtx.lineWidth = 4; rewardTextCtx.font = `bold ${nameFontSize}px sans-serif`;
            const nameText = `${gameState.playerName}選手`;
            const nameX = rewardTextCanvas.width / 2, nameY = rewardTextCanvas.height - (canvasHeight * 0.1);
            rewardTextCtx.strokeText(nameText, nameX, nameY); rewardTextCtx.fillText(nameText, nameX, nameY);

            const statsFontSize = canvasWidth * 0.045;
            rewardTextCtx.lineWidth = 3; rewardTextCtx.font = `bold ${statsFontSize}px sans-serif`;
            const stats = gameState.stats;
            const statsText = `KICK:${stats.kick} / SPEED:${stats.speed} / IQ:${stats.iq}`;
            const statsX = rewardTextCanvas.width / 2, statsY = rewardTextCanvas.height - (canvasHeight * 0.05);
            rewardTextCtx.strokeText(statsText, statsX, statsY); rewardTextCtx.fillText(statsText, statsX, statsY);
        };
